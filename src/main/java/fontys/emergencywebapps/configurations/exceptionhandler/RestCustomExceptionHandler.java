package fontys.emergencywebapps.configurations.exceptionhandler;
...
public class RestCustomExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<Object> handleMaxUploadSizeExceededException(MaxUploadSizeExceededException e) {
        log.error("Max upload size exceeded exception occurred: " + e.getMessage());

        // Create a list with the custom message and the message from MaxUploadSizeExceededException
        List<ValidationErrorDTO> errors = new ArrayList<>();
        errors.add(new ValidationErrorDTO(null, e.getMessage()+". Max upload size is 10MB"));
    ...
    public ResponseEntity<Object> handleDataIntegrityViolationException(DataIntegrityViolationException e) {
        ...
        // Create a list with the custom message and the message from DataIntegrityViolationException
        List<ValidationErrorDTO> errors = new ArrayList<>();
        errors.add(new ValidationErrorDTO(null, "Data integrity violation exception occurred. " +  e.getMessage()));
